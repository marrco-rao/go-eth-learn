...
func handleBalanceOf(ctx context.Context, client *ethclient.Client, parsedABI abi.ABI, contractHex string, addrHex string) {
	if contractHex == "" || addrHex == "" {
		log.Fatal("balance mode requires --contract and --address flags")
	}
	...
	if err != nil {
		log.Fatalf("Failed to pack balanceOf data: %v", err)
	}
	...
	if err != nil {
		log.Fatalf("Failed to unpack balanceOf result: %v", err)
	}
	fmt.Printf("=== ERC-20 BalanceOf Result ===\n")
...
// handleTransfer - 发送 ERC-20 转账交易
func handleTransfer(ctx context.Context, client *ethclient.Client, parsedABI abi.ABI, contractHex string, toHex string, amountStr string) {
	if contractHex == "" || toHex == "" || amountStr == "" {
		log.Fatal("transfer mode requires --contract, --to, and --amount flags")
	}

	// 获取私钥
	privateKeyHex := os.Getenv("SENDER_PRIVATE_KEY")
	if privateKeyHex == "" {
		log.Fatal("SENDER_PRIVATE_KEY environment variable is not set")
	}
	// 解析私钥
	...
	// 增加 20% 的 Gas 作为缓冲，避免因 Gas 不足导致交易失败
	gasLimit = uint64(float64(gasLimit) * 1.2)

	// 获取建议的 Gas 价格（使用 EIP-1559 动态费用）
	gasTipCap, err := client.SuggestGasPrice(ctx)
	if err != nil {
	...
	if balance.Cmp(totalGasCost) < 0 {
		log.Fatalf("Insufficient balance to cover gas fees: balance %s Wei, required %s Wei", balance.String(), totalGasCost.String())
	}
	...
	txData := &types.DynamicFeeTx{
		...
		GasFeeCap: gasFeeCap,
		Gas:       gasLimit,
		To:        &contractAddr, // 交易发送到 ERC-20 合约地址
		Value:     big.NewInt(0), // ERC-20 转账不发送 ETH
		Data:      callData,
	}
...
func waitForTransaction(ctx context.Context, client *ethclient.Client, txHash common.Hash) {
	...
	for {
		select {
		case <-waitCtx.Done():
			fmt.Println("Timed out waiting for transaction confirmation.")
			fmt.Println("You can check the transaction status later")
		...
		case <-ticker.C:
			...
			if err == ethereum.NotFound {
				fmt.Printf("Transaction not yet mined. Hash: %s\n", txHash.Hex())
				continue
			} else if err != nil {
				// 交易可能还在 pending 状态
				continue
			...
			if receipt.Status == 0 {
				...
				if len(receipt.Logs) > 0 {
					fmt.Printf("\nTo Parsing Transfer event from this transaction:\n")
					fmt.Printf("go run main.go --mode parse-event --tx %s\n", txHash.Hex())
				}
			}
			fmt.Printf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
...
func handleParseEvent(ctx context.Context, client *ethclient.Client, parsedABI abi.ABI, txHashHex string) {
	if txHashHex == "" {
		log.Fatal("parse-event mode requires --tx flag")
	}
	...
	for i, vLog := range receipt.Logs {
		// 	检查 Topic[0] 是否匹配 Transfer 事件的签名哈希
		...
		if len(vLog.Data) > 0 {
			...
			if err != nil {
				...
				fmt.Printf("Decoded Non-Indexed Parameters:\n")
			...
			fmt.Printf("No Data (non-indexed parameters are empty)\n")
		}
		fmt.Printf("\n")
		...
		if len(vLog.Topics) >= 3 {
			...
			if value != nil {
				fmt.Printf("Transfer Event Details:\n")
				fmt.Printf("  From : %s\n", fromAddr.Hex())
				fmt.Printf("  To   : %s\n", toAddr.Hex())
				fmt.Printf("  Value: %s (raw uint256)\n", value.String())
			}
		}
		fmt.Printf("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n")
		fmt.Printf("\n")
	...
	if !foundTransfer {
		fmt.Printf("No Transfer events found in this transaction.\n")
